...
## Leaving 'game_seed' value as -1 will let the game pseudo-randomly select a different seed.
@export var game_seed: int = -1
@export_range(0.0, 1.0, 0.01) var alive_bias = 0.1

@export_category("Settings")
@export var fps_limit: int = 5
...
func _display_map(current_map: Dictionary) -> void:
	for current_position: Vector2i in current_map.keys():
		self.call_deferred("add_child", tile_map.get(current_position))
...
func _generate_map(current_map: Dictionary) -> Dictionary:
	...
	for current_position: Vector2i in current_map.keys():
		...
		# If no values from user, do gamerules.
		mutex.lock()
...
func _initialize_world() -> void:
	...
	UI.call_deferred("set_seed_label_text", "Game seed: %s" % [rng.get_seed()])
...
func _update_map(current_map: Dictionary) -> void:
	if UI.get_generation_label(): # When label is ready.
		UI.set_generation_label_text("Generation: %s" % [generation_counter])
